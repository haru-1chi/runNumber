...
$columns = [
    0 => 'c.computer_center_number',
    1 => 'c.asset_number',
    2 => 'c.computer_name',
    3 => 'd.depart_name',
    4 => 'c.equipment_location'
];
...
$totalRecordsQuery = "SELECT COUNT(*) FROM run_number.computer_assets WHERE is_deleted != 1";
$totalRecordsStmt = $conn->prepare($totalRecordsQuery);
$totalRecordsStmt->execute();
$totalRecords = $totalRecordsStmt->fetchColumn();

// Count total records after filtering
$totalFilteredQuery = "SELECT COUNT(*) FROM run_number.computer_assets c
    LEFT JOIN OrderIT.depart d ON c.department = d.depart_id
    WHERE c.is_deleted != 1 $searchQuery";
...
$sql = "SELECT c.*, d.depart_name 
        FROM run_number.computer_assets c
        JOIN (
            SELECT computer_center_number, MAX(timestamp) AS max_timestamp
            FROM run_number.computer_assets
            WHERE is_deleted != 1
            GROUP BY computer_center_number
        ) latest 
        ON c.computer_center_number = latest.computer_center_number 
